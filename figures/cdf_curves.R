...
data_fits <- read.csv(file.path(DataFitDir, 'czo regolith data dist fits.csv'), stringsAsFactors = FALSE) #this analysis was completed in " .xlsx"
head(data_fits)

gray_colors <- gray.colors(n=4, start = 0.05, end = 0.75)
lwd_setting <- 2

tiff(file = file.path(FiguresDir, 'cdf0_8_data_distributions.tif'), family = 'Times New Roman', pointsize = 11, width = 3.25, height = 3.5, units = 'in', res=res_plots, compression = 'lzw')
...
tiff(file = file.path(FiguresDir, 'cdf0_40_data_distributions.tif'), family = 'Times New Roman', pointsize = 11, width = 3.25, height = 3.5, units = 'in', res=res_plots, compression = 'lzw')
par(mar=c(4.5, 4.5, 0.5, 0.5))
